...

{
	...
	if (success == 0)
	{
		...
		while(temp_addr != NULL)
		{
			...
			{
				const struct sockaddr_in* dlAddr = (const struct sockaddr_in *)temp_addr->ifa_addr;
                const uint8_t* base = (const uint8_t*)&dlAddr->sin_addr;
                NSMutableString* ip = [[NSMutableString new] autorelease];
                for (int i = 0; i < 4; i++) 
                {
                    if (i != 0) 
                        [ip appendFormat:@"."];
                ...
                [result setObject:(NSString*)ip forKey:[NSString stringWithFormat:@"%s", temp_addr->ifa_name]];
	...
	return [[result allValues] description];
}
// ----------------------------------------------------------------------------------------------------------
